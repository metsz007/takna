...
# Plan 04 — Alarm history / fired log
...
"Last fired" line — so the user can answer "did my 8am alarm actually fire?"
without trusting the OS blindly.

## Prereq

None (plans 01 and 02 are done; 03 is independent). No new runtime
dependencies — drift, the DB, and `intl` are already here. Requires a
`build_runner` regen (new table → regenerated `database.g.dart`).

## Design decisions (why a log, why not reconcile)

- **Append-only audit log, deliberately outside the single write path.**
  CLAUDE.md's rule is UI → repository → DB → `reconcile()`. A fired-event row is
  not source-of-truth scheduling state — it is a historical fact ("this alarm
...
- **Store a title snapshot, and that is NOT "derived data".** The
  ...
  still read "Standup — dismissed". A recorded fact at time-of-fire is not
  derivable after the fact, so it is stored, once, on the event row.
- **`kind` is a plain text tag** (`'fired'` / `'dismissed'` / `'snoozed'`) —
...
- **Retention: prune-on-insert, 90-day cap.** `logFired` deletes rows older
  ...
  job — alarms are infrequent, one extra `DELETE` per fire is free. Ceiling:
...
- **Laziest UI: a "Last fired" line on the existing detail screen.** No new
  screen, no new route, no settings entry — the trust question is per-reminder,
...
- `lib/core/database/database.dart` — add the `FiredEvents` table (6-24 area),
  register it in `@DriftDatabase(tables: [Reminders, FiredEvents])` (26), bump
  `schemaVersion` 3 → 4 (37) with a `createTable` migration (40-45), and add
  `logFired` + `lastFired` methods (alongside `setSnoozedUntil` etc., 62-68).
- `lib/core/database/database.g.dart` — **regenerated** by build_runner (do not
  hand-edit).
- `lib/core/notifications/notification_service.dart` —
  ...
  `AppDatabase` — the DB uses `shareAcrossIsolates`, so the insert is safe.
- `lib/features/reminders/presentation/screens/alarm_screen.dart` —
  ...
  already covers those verbs — see note in step 4).
...
- `test/fired_events_test.dart` — **new**, sits alongside `scheduler_test.dart`
  / `notification_action_test.dart`.

## Steps

1. **Table + DB methods — `database.dart`.**
   ```dart
   ...
   // old row. Not reconcile-routed on purpose — append-only audit log.
   Future<void> logFired(String reminderId, String title, String kind,
     ...
     // ponytail: prune-on-insert, 90-day cap — no background job.
...
2. **Log the terminal action — `notification_service.dart`.** In
   `handleNotificationAction`, after the existing DB effect and before/after the
   `reconcile()` (order doesn't matter — separate table), record the verb:
   ```dart
   ...
   This covers **shade action buttons in every state** — foreground dispatch and
   the dead-app background isolate — because both route through this function.

3. **Log the ring — `alarm_screen.dart` `initState`.** The full-screen alarm
   ...
   ```
   `databaseProvider` is already reachable via `ref` here. Guard nothing extra —
...
4. **Do NOT double-log the full-screen Snooze/Dismiss buttons.**
   ...
   shade buttons — a single fire is resolved by exactly one of them. The
...
6. **Detail line — `reminder_detail_screen.dart`.** Under the NEXT ALARM
   ...
   ```
   Last fired: Today 8:00 AM · dismissed
   ```
   Reuse the existing `_dayLabel` + `DateFormat('h:mm a')` and `body(...)`
   styling (`t.ink3`). Render nothing when the event is null or still loading —
   no "never fired" placeholder (lazy; absence reads as "hasn't rung yet").

## Done when

- `test/fired_events_test.dart` (new) passes — pure, in-memory
...
- A full scrollable history list / a Settings "History" screen — the per-
...
- Logging fires that the user **never interacts with** (alarm rings, user swipes
  ...
  reminders that are swiped away log nothing — `flutter_local_notifications`
